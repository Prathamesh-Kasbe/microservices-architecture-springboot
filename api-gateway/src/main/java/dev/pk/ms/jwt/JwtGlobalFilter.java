 ...
 * 
 * @Component public class JwtGlobalFilter implements GlobalFilter, Ordered {
 * 
 ...
 * System.out.println("In Bearer check________________________________"); return
 * onError(exchange, "Missing or invalid Authorization header",
 * HttpStatus.UNAUTHORIZED); }
 * 
 * String token = authHeader.substring(7); try { Claims claims =
 * jwtUtil.extractClaims(token);
 * 
 * List<String> roles = claims.get("roles", List.class); String path =
 * exchange.getRequest().getPath().toString();
 * 
 * if (path.contains("/customer") && !roles.contains("CUSTOMER")) { System.out
 * .println("In customer Before updating exchange in api gateway________________________________"
 * ); return onError(exchange, "Forbidden", HttpStatus.FORBIDDEN); } else if
 * (path.contains("/account") && !roles.contains("ACCOUNT")) { System.out
 ...
 * ); e.printStackTrace(); return onError(exchange, "Invalid token",
 * HttpStatus.UNAUTHORIZED); } // Add Internal secret to header
 * ServerHttpRequest mutatedRequest = exchange.getRequest().mutate()
 * .header("X-Internal-Secret", jwtProperties.getSecret()).build();
 * 
 * ServerWebExchange mutatedExchange =
 * exchange.mutate().request(mutatedRequest).build(); System.out.
 * println("After updating exchange in api gateway________________________________"
 * ); return chain.filter(mutatedExchange); }
 * 
 * private Mono<Void> onError(ServerWebExchange exchange, String err, HttpStatus
 * status) { System.out.println("Invalid token detected, returning 401");
 * exchange.getResponse().setStatusCode(status); return
 * exchange.getResponse().setComplete(); }
 * 
 * @Override public int getOrder() { return -1; // high priority (runs early) }
 * }
 */